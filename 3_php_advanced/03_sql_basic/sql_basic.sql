...
FROM
	countries AS cntr
  LEFT JOIN countrylanguages AS cntr_lang
  	ON cntr.code = cntr_lang.country_code
;
...
FROM
  ...
  LEFT JOIN countries AS cntr
  	ON clbt.country_code = cntr.code
;
...
FROM
  ...
  JOIN countries AS cntr
  	ON clbt.country_code = cntr.code
  JOIN countrylanguages AS cntr_lang
  	ON clbt.country_code = cntr_lang.country_code AND
      cntr_lang.is_official = 'T'
;
...
SELECT
	clbt.country_code,
    MAX(clbt.age) AS max_age,
    MIN(clbt.age) AS min_age
FROM
	celebrities AS clbt
WHERE
	clbt.country_code != ''
...
SELECT
    cntr.name AS '国名',
    AVG(clbt.age) AS '平均年齢'
FROM
	countries AS cntr
  JOIN celebrities AS clbt
  	ON cntr.code = clbt.country_code
GROUP BY
	cntr.code
ORDER BY
	AVG(clbt.age) DESC
